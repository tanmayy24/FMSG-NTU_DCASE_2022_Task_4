...
class PANNS_GRU(nn.Module):
    ...
    def forward(self, x, pad_mask=None, temp=1):
        ...
        # else:
        #     weak = strong.mean(1)
        #Exponential Softmax Pooling layer
        weak = (strong * strong.exp()).sum(dim = 1) / strong.exp().sum(dim = 1)
    ...
    def train(self, mode=True):
        ...
        """
        super(PANNS_GRU, self).train(mode)
        if self.freeze_bn:
