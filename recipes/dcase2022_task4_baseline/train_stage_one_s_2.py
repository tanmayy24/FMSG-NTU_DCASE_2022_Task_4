...
from desed_task.dataio.datasets import StronglyAnnotatedSet, UnlabeledSet, WeakSet, StronglyToWeakAnnotatedSet
from desed_task.nnet.PANNS_GRU import PANNS_GRU
from desed_task.utils.encoder import ManyHotEncoder
from desed_task.utils.schedulers import ExponentialWarmup

from local.classes_dict import classes_labels
from local.sed_trainer_stage_one_s_2 import SEDTask4
...
def single_run(
    ...
    ##### model definition  ############
    sed_student = PANNS_GRU(**config["net"])
    pretrained_checkpoint_path = 'config["data"]["PANNS_pretrained_weight"]'
    sed_student.load_from_pretrain(pretrained_checkpoint_path)

    if test_state_dict is None:
        ##### data prep train valid ##########
        synth_df = pd.read_csv(config["data"]["synth_tsv"], sep="\t")
        synth_set = StronglyToWeakAnnotatedSet(
        ...
        if strong_real:
            ...
            strong_set = StronglyToWeakAnnotatedSet(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_file",
        default="./confs/sed_stage_one_s_2.yaml",
